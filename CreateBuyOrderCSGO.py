...
url = 'https://bitskins.com/api/v1/get_market_buy_orders/?api_key=b0aec946-4b15-4985-99fa-98e0b1b1dec4&code=' + totp.now() + '&market_hash_name=Jeans (Tan)&page=2,3&app_id=730'
r = requests.get(url)
...
for item in loot_filt:
    ...
    if  (3 < price < bal) and (name in d.keys()):
        ...
        url = 'https://bitskins.com/api/v1/get_sales_info/?api_key=b0aec946-4b15-4985-99fa-98e0b1b1dec4&market_hash_name=' + name +'&page=1&app_id=730&code=' + totp.now()
        r = requests.get(url)
        parsed_string = r.json()
        k = 0
        f = False
        ...
        if k > 10 and f:
            ...
            url = 'https://bitskins.com/api/v1/create_buy_order/?api_key=' + api_key + '&code=' + totp.now() + '&name=' + name + '&price=' + str(round(price,2)) + '&quantity=1&app_id=730'
            r = requests.get(url)
            bal -= price
