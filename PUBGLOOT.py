...
url2 = 'https://loot.farm/fullpricePUBG.json'

bitskloot_cost_filt = []

while True:
    try:
        url1 = 'https://bitskins.com/api/v1/get_price_data_for_items_on_sale/?api_key=b0aec946-4b15-4985-99fa-98e0b1b1dec4&code=' + totp.now() + '&app_id=578080'
        ...
        for item in parsed_string_bitsk["data"]["items"]:
            if 0.2 <= float(item["lowest_price"]) <= 50:  # Фильтр по цене bits
        ...
        for item in loot_filt:
            ...
            for k in bitsk_cost_filt:
                ...
                if (nameBit == name):
                    dim = (float(k["lowest_price"]) / (float(item["price"]) * 0.97) * 100 - 1)
                    if (dim <= -0.3) and (k not in bitskloot_cost_filt):
                        f = open('textPUBG.txt', 'a')
                        f.write(str(round(dim, 2)) + '\n')
                        f.write(name + '\n')
